      ...
      <div class="col-md-4 col-sm-12 text-center">
        ...
        <form method="post">
          ...
          <div class="left-desk login">
            <a href="{{route('home')}}"><img src="{{asset('img/logo3.png')}}" height="50px"></a>
            <h4>Admin Login</h4>
            @if(Session::get('msg'))
            ...
            @endif
            <input id="email" type="email" class="login-input2{{ $errors->has('email') ? ' is-invalid' : '' }}" name="email" value="{{ old('email') }}" placeholder="Enter email" required autofocus style="max-width:304px;">
            {{ $errors->first('email') }}
            <input id="password" type="password" class="login-input{{ $errors->has('password') ? ' is-invalid' : '' }}" name="password" placeholder="Enter password" required style="max-width:304px;">
